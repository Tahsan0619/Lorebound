...
function estimateSessionSteps(category, payload) {
    if (category === 'Timeline') {
        return payload.levels.reduce((sum, lvl) => {
            ...
            return sum + n + drills + n + finales;
        }, 0);
    }
    if (category === 'Process') {
        return payload.phases.reduce((sum, ph) => {
            const n = ph.stages.length;
            return sum + n + 1 + n + Math.max(1, n - 1);
        }, 0);
    }
    ...
    return cards + traps + Math.ceil(memory / 2) + debates;
}
...
function estimateLevelSteps(category, levelPayload) {
    if (category === 'Timeline') {
        ...
        const drills = Math.max(1, (n - 1) + (n - 1));
        const finales = Math.max(1, n - 2);
        return n + drills + n + finales;
    }
    if (category === 'Process') {
        const n = levelPayload.stages.length;
        return n + 1 + n + Math.max(1, n - 1);
    }
    if (category === 'CauseEffect') {
        const n = levelPayload.chains.length;
        return n + n + 1 + Math.max(1, n - 1);
    }
...
function loadDemoCards() {
    ...
    if (!list || !window.LoreboundAPI) {
        renderFallbackDemoCards();
        return;
    }

    LoreboundAPI.request('/samples').then(res => {
        const slugToKey = Object.fromEntries(
            Object.entries(LoreboundAPI.sampleSlugs).map(([k, v]) => [v, k])
        );

        list.innerHTML = res.data.map((sample, idx) => {
            const key = slugToKey[sample.slug] || String(idx + 1);
            const badge = window.LoreboundFX?.badgeClass(sample.category) || 'timeline-badge';
            const icon = sample.theme?.icon || LoreboundFX?.mechanicIcon(sample.category)?.replace('fa-', '') || 'gamepad';
...
function renderFallbackDemoCards() {
    ...
    if (!list) return;
    const demos = [
        { key: '1', emoji: '💻', badge: 'timeline-badge', icon: 'microchip', mechanic: 'Timeline Builder', title: 'Internet & Computing Revolution', topic: 'Computer Science • Grade 9-12', color: '#00f2fe' },
        { key: '2', emoji: '❤️', badge: 'cycle-badge', icon: 'heart-pulse', mechanic: 'Process Loop', title: 'Blood Journey Through the Human Heart', topic: 'Biology • Grade 10', color: '#ff4081' },
        { key: '3', emoji: '🌊', badge: 'cause-badge', icon: 'water', mechanic: 'Cause-Effect Chain', title: 'Ocean Plastic Pollution Chain', topic: 'Marine Ecology • Grade 11', color: '#26c6da' },
        { key: '4', emoji: '⚡', badge: 'sorting-badge', icon: 'bolt', mechanic: 'Comparison Sorter', title: 'Renewable vs Fossil Fuels', topic: 'Energy & Civics • Grade 10', color: '#69f0ae' },
    ];
...
function bindSampleCards() {
    document.querySelectorAll('.sample-item-card').forEach(card => {
...
function runCompilationPipeline(text, sourceTitle, preloadedSample = null, sampleKey = null) {
    ...
    const finishCompilation = (compiledSample) => {
        ...
        const finalTitle = compiledSample.title || sourceTitle;
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                if (localSample) {
                    ...
                    finalPayload = generateDynamicPayload(text, AppState.activeCategory);
                }
    ...
    setTimeout(async () => {
        ...
        try {
            ...
            const compiled = await window.runApiCompilation(text, sourceTitle, sampleKey);
...
function startGameSession(category, payload, metadata, title) {
    ...
    const descMap = {
        Timeline: 'Four phases per era: build the timeline, solve detective quizzes, master the sequence from memory, and bridge missing events.',
        Process: 'Navigate the cycle, reorder stages, tune simulation parameters, and lock the causal chain between stages.',
        CauseEffect: 'Link causes to effects, predict ripples, assemble the chain order, and defeat inference boss challenges.',
        Comparison: 'Sort attributes, hunt trap statements, memory-match categories, and win policy debate scenarios.',
    };
    UI.narrativeText.textContent = theme.narrative_intro || descMap[category] || descMap.Timeline;

    UI.gameActiveTitle.textContent = title;
    UI.gameActiveDesc.textContent = UI.narrativeText.textContent;

    const fxIcon = theme.icon ? `fa-${theme.icon}` : (LoreboundFX?.mechanicIcon(category) || 'fa-gamepad');
...
function updateStepHUD() {
    ...
    UI.gameStepCounter.textContent = `${depthLabel} • Level ${AppState.currentLevelIndex + 1}/${AppState.totalLevelsCount} • ${sessionCurrent}/${sessionTotal} challenges • ${mins}m elapsed`;
}
